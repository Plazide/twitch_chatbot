...
async function song(params){
	...
	const entry = {
		...
		title: helper.htmlDecode(video.snippet.title),
		duration: helper.iso2ms(video.duration),
		user: chatter
	}
...
async function clearsongs(params){
	...
	if(current.playlist.length === 0){
		...
		const data = {
			topic: "PLAYLIST",
			playlist: newer.playlist
		}

		ws.sendToClient(newer.sess_id, data);
	}
...
async function regretsong(params){
	...
	if(!foundOne){
		...
		ws.sendToClient(user.sess_id, data);
	}

...
async function clearsongs(params){
	...
	if(current.playlist.length === 0){
		client.say(channel, `@${chatter}, the playlist is already empty!`);
	}else if(newer.playlist.length === 0){
		client.say(channel, `@${chatter}, the playlist was cleared!`);
	}else if(newer.playlist.length > 0){
		client.say(channel, `@${chatter}, it seems like something went wrong. The playlist was not cleared.`);
	}
